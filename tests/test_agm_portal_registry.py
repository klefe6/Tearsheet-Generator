...
def test_algominds_account_remains_active_tearsheet_account():
    ...
    assert algominds["fee_tier"] == registry.FEE_TIER_LABEL


def test_fernando_and_kaladhar_units_left_blank():
    rows = registry.build_participating_accounts(**_sample_live_kwargs())
    by_name = {row["account"]: row for row in rows}
    assert by_name["Dr. Rajeev Fernando"]["units"] is None
    assert by_name["Kaladhar Palaniappan"]["units"] is None


def test_other_listed_investors_have_units_one():
    rows = registry.build_participating_accounts(**_sample_live_kwargs())
    unknown_units = {"Dr. Rajeev Fernando", "Kaladhar Palaniappan"}
    for row in rows:
        if row["account"] in unknown_units:
            continue
...
def test_onboarding_status_values():
    rows = registry.build_participating_accounts(**_sample_live_kwargs())
    by_name = {row["account"]: row["status"] for row in rows}
    assert by_name["Ramachandran Kuppusamy"] == "7/12"
    assert by_name["Pridhiraj Ulaganathan"] == "7/12"
    for name, status in by_name.items():
        if name in ("Ramachandran Kuppusamy", "Pridhiraj Ulaganathan"):
            continue
        assert status == "12/12", name
...
def test_non_live_accounts_do_not_guess_financial_fields():
    ...
    assert srinivas["fee_tier"] is None
