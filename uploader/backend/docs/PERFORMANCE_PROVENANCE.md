...
When the backend is reachable, `program_data_source` is one of:

| Value | Meaning |
| ----- | ------- |
| `uploader_daily_rows` | Every plotted point is one of Glenn's uploader entries |
| `uploader_daily_rows+tearsheet_backfill` | At least one plotted point was imported from tearsheet history (`historical_rows` — see `historical_backfill.md`) |

| Question | Answer |
| -------- | ------ |
| Derived from uploader storage? | **Yes** — `db.get_merged_rows(program)`: manual `daily_rows` merged with backfilled `historical_rows` |
| Normalized from entered NLV? | **Yes** — `program_nlv()` per program, compounded to $100k base |
| Only uploader-entered rows? | Only when no backfill was imported; per-series `backfilled_point_count` / `manual_point_count` report the split |
| Manual vs imported precedence | A manual entry **always** supersedes a backfilled row on the same date |
| Reads live tearsheet apps? | **No** — never imports tkp_ts / tcp_ts_v2 / mp_ts; backfill is an offline read-only extract, sandbox-only import |
| History before first uploader row? | Only if backfilled; otherwise series starts at first stored row |
| Missing dates | Only stored dates; no gap-filling |
| AGM `fee` | **Excluded** from performance (documented placeholder; also not backfilled) |
