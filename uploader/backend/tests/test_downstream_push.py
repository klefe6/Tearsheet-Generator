...
def test_unreachable_ingest_is_clean_failure():
    client = _push_client("http://127.0.0.1:9/api/uploader/ingest-daily-row", dry_run=False)
    try:
        client.post("/api/rows/TCP", json=VALID_ROWS["TCP"])
        r = client.post("/api/export/all")
        result = r.json()["downstream"]["results"]["TCP"]
        assert result["status"] == "failure"
        assert "not reachable" in result["date_results"][0]["reason"]
        assert client.get("/api/rows/TCP").json()["rows"][0]["exported"] is False
    finally:
        client.close()
